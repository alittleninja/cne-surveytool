	...
	*/
    private function getExpectationData($results) {
	    if(count($results) <= 0) return;
		    $expectData = array (0, 0, 0, 0, 0);
		
		    $surveySum = array("brownbag"=>0,"workshop"=>0,"infosession"=>0,"bootcamp"=>0, "techseries" => 0);
	    foreach($results as $surveyGroup) {
		    $expectationSum = 0;
	        ...
	        }else if($surveyGroup['SurveyGroup']['type'] === 'Boot Camp'){
		        $expectData[3] += $expectationSum;
		        $surveySum["bootcamp"] += count($surveyGroup['Survey']);
	        }
        }
	    ...
	    if($surveySum["bootcamp"] != 0){$expectData[3]/=$surveySum["bootcamp"];}
    ...
    public function expectationsBar($expectationData) {
        ...
        $this->HighCharts->setChartParams (
                        ...
                        (
                            ...
                            'xAxisLabelsEnabled' 			=> TRUE,
                            'xAxisCategories'       	=> array( 'Brown Bag', 'Workshop', 'Info Session', 
								'Boot Camp', 'Tech Series'),
